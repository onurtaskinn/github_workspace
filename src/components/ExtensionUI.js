...
const ExtensionUI = () => {
    ...
    useEffect(() => {
        ...
        // �ifreli i�erik ��zme iste�i
        if (window.showDecryptionView && isAuthenticated) {
            console.log('Opening decryption view from background');
            setShowDecryption(true);
            setEncryptedContent(window.encryptedEmailContent || '');
            window.showDecryptionView = false;
            window.encryptedEmailContent = null;
        }
    ...
    const renderMainScreen = () => {
        // E�er �ifre ��zme g�r�n�m� g�steriliyorsa
        if (showDecryption) {
            return (
